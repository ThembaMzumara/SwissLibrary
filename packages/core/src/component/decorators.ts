...
const SWISS_EVENT_HANDLERS = Symbol('swiss:event-handlers');
...
function createEventDecorator(eventType: string) {
  return function(selectorOrOptions?: string | EventHandlerOptions, options?: EventHandlerOptions) {
    return function(target: object, propertyKey: string, descriptor: PropertyDescriptor) {
      let finalOptions: EventHandlerOptions;
      ...
      if (typeof selectorOrOptions === 'string') {
        ...
        finalOptions = selectorOrOptions || {};
        selector = finalOptions.selector;
      }

      // Apply throttling/debouncing if specified
      if (descriptor && descriptor.value && (finalOptions.throttle || finalOptions.debounce)) {
        const originalMethod = descriptor.value;
        if (finalOptions.throttle) {
          descriptor.value = applyThrottle(originalMethod, finalOptions.throttle);
        }
        // TODO: Add debounce implementation if needed
      }

      // Store event handler metadata
      attachMetadata(target, SWISS_EVENT_HANDLERS, {
        eventType,
        method: propertyKey,
        selector,
...
// Common DOM event decorators
export const onClick = createEventDecorator('click');
export const onChange = createEventDecorator('change');
export const onInput = createEventDecorator('input');
export const onSubmit = createEventDecorator('submit');
export const onFocus = createEventDecorator('focus');
export const onBlur = createEventDecorator('blur');
export const onKeyDown = createEventDecorator('keydown');
export const onKeyUp = createEventDecorator('keyup');
export const onMouseOver = createEventDecorator('mouseover');
export const onMouseOut = createEventDecorator('mouseout');
export const onMouseEnter = createEventDecorator('mouseenter');
export const onMouseLeave = createEventDecorator('mouseleave');
...
export function getEventHandlerMetadata(target: object) {
  return Reflect.getMetadata(SWISS_EVENT_HANDLERS, target) || [];
}
...
export function applyDecoratorMetadata(component: SwissComponent) {
  ...
  const eventHandlers = getEventHandlerMetadata(constructor) as Array<{
    eventType: string;
    method: string;
    selector?: string;
  ...
  // Store event handlers for later DOM binding (when component mounts)
  (component as unknown as { _swissEventHandlers?: unknown[] })._swissEventHandlers = eventHandlers;
}
...
export function collectComponentMetadata(target: object) {
  return {
    ...
    eventHandlers: getEventHandlerMetadata(target)
  };
}

...
const componentInstances = new WeakMap<Node, SwissComponent>();

// Type guards for robust type handling
function isTextVNode(vnode: VNode): vnode is Extract<VNode, string | number> {
  ...
  return (
    ...
    (typeof vnode === "object" &&
      ...
      ((vnode.type as unknown) === Fragment ||
        (typeof vnode.type === "symbol" && (vnode.type as any).toString() === "Symbol(Fragment)")))
  );
}
...
function createDOMNode(vnode: VNode): Node {
  try {
    // Unwrap Signal objects automatically
    if (isSignal(vnode)) {
      return createDOMNode(vnode.value as VNode);
    }
    ...
    if (isTextVNode(vnode)) {
      ...
      const rendered = renderComponent(vnode);
      const dom = createDOMNode(rendered);
      // Store the component instance for future updates
      const Component = vnode.type;
      if (isClassComponent(Component)) {
        const instance = (rendered as any).__componentInstance;
        if (instance) {
...
function createTextNode(vnode: string | number): Text {
...
function renderComponent(vnode: ComponentVNode, existingInstance?: SwissComponent): VNode {
  try {
    const Component = vnode.type;
    ...
    if (typeof Component === "function") {
      if (isClassComponent(Component)) {
        ...
        if (existingInstance) {
          // Reuse existing instance and update props
          instance = existingInstance;
          instance.props = props;
        } else {
          // Create new instance
          instance = new Component(props);
          // Initialize component (now that fields are set)
          if (typeof (instance as any).initialize === 'function') {
            (instance as any).initialize();
          }
        }

        // Render with untrack to prevent parent effect from tracking child signals
        // The child component will set up its own effect in initialize()
        const rendered = untrack(() => instance.render());

        // Store VNode on instance for reactivity
        (instance as any)._vnode = rendered;
        // Store instance on VNode for retrieval
...
export function renderToDOM(vnode: VNode, container: HTMLElement) {
  try {
    if (
      ...
      "dom" in vnode &&
      vnode.dom &&
      vnode.dom.parentNode === container
    ) {
...
export function updateDOMNode(dom: Node, vnode: VNode) {
  try {
    // Unwrap Signal objects automatically
    if (isSignal(vnode)) {
      updateDOMNode(dom, vnode.value as VNode);
      return;
    }
...
function updateTextNode(dom: Text, vnode: string | number) {
...
function updateComponentNode(
  ...
  if (oldVNode && isComponentVNode(oldVNode) && oldVNode.type === vnode.type) {
    // Component update logic - reuse existing instance
    const existingInstance = componentInstances.get(dom);
    const oldRendered = vnodeMetadata.get(dom);
    const newRendered = renderComponent(vnode, existingInstance);

    // Store the instance from the new render
    const newInstance = (newRendered as any).__componentInstance;
    if (newInstance) {
...
function updateEventListener(
  ...
  if (existingHandler) {
    console.log(`[Renderer] Removing event listener: ${eventName} from`, element);
    element.removeEventListener(eventName, existingHandler);
  }

  if (typeof newHandler === "function") {
    console.log(`[Renderer] Adding event listener: ${eventName} to`, element);
